...
async def check_eligibility(request: dict):
    """Check user eligibility against all schemes and return a formatted summary."""
    try:
        ...
        # Prepare summary text via LLM (best-effort)
        try:
            eligible_payload = [e.dict() if hasattr(e, 'dict') else e for e in eligible_schemes]
            near_payload = [n.dict() if hasattr(n, 'dict') else n for n in near_misses]
            summary = await OpenRouterClient.format_eligibility_text(eligible_payload, near_payload)
        except Exception as e:
            logger.warning(f"Summary formatting failed: {e}")
            summary = ""
        
        return {
            "eligible_schemes": eligible_payload,
            "near_misses": near_payload,
            "summary": summary
        }
